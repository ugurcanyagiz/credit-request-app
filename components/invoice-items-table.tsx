...
export function InvoiceItemsTable({ items, customerCode, invoiceNo }: InvoiceItemsTableProps) {
  ...
  async function addNotFromRecentInvoicesNote() {
    setIsSubmitting(true);
    setSubmitError(null);
    ...
    const response = await fetch("/api/cart", {
      ...
      body: JSON.stringify({
        ...
        item_descp: "Please add credit note if the item is not from the invoice above",
        quantity: 0,
        sales_amount: 0,
    ...
    <>
      ...
      <div className="mt-5 rounded-md border border-zinc-200 p-4">
        ...
        <p className="mt-1 text-sm text-zinc-600">
          Please add credit note if the item is not from the invoice above
        </p>
        <div className="mt-3">
          <button
            type="button"
            onClick={() => void addNotFromRecentInvoicesNote()}
